    ...
    <div>
        ...
        <div class="container">
            ...
            <el-table :data="tableData" border class="table" ref="multipleTable" header-cell-class-name="table-header"
                ...
                <el-table-column prop="meetingId" label="会诊记录ID"></el-table-column>
                <el-table-column prop="initiatorId" label="发起人ID"></el-table-column>
                <el-table-column prop="recipientId" label="接收人ID"></el-table-column>
                <el-table-column prop="msgType" label="消息类型"></el-table-column>
                <el-table-column prop="createTime" label="创建时间"></el-table-column>
                <el-table-column prop="status" label="是否删除"></el-table-column>
...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getData() {
                axios.post(this.baseURL + '/session/search/' + this.currPage + '/' + this.pageSize, this.vo).then(res => {
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        axios.delete(this.baseURL + '/session/' + row.id).then(res => {
                            if (res.data.flag) {
                                this.getData();
            ...
            delAllSelection() {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        axios.post(this.baseURL + '/session/delByIds', ids).then(res => {
                            if (res.data.flag) {
                                this.getData();
            ...
            saveEdit() {
                ...
                if (this.form.id == null) {
                    result = axios.post(this.baseURL + '/session', this.form);
                } else {
                    result = axios.put(this.baseURL + '/session', this.form);
                }
                // 处理结果
                result.then(res => {
                    if (res.data.flag) {
