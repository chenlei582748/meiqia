...
<script>
    ...
    export default {
        ...
        methods: {
            // 获取 easy-mock 的模拟数据
            getData() {
                axios.post(this.baseURL + '/level/search/' + this.currPage + '/' + this.pageSize, this.vo).then(res => {
            ...
            handleDelete(index, row) {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        axios.delete(this.baseURL + '/level/' + row.id).then(res => {
                            if (res.data.flag) {
                                this.getData();
            ...
            delAllSelection() {
                ...
                this.$confirm('确定要删除吗？', '提示', {
                    ...
                    .then(() => {
                        axios.post(this.baseURL + '/level/delByIds', ids).then(res => {
                            if (res.data.flag) {
                                this.getData();
            ...
            saveEdit() {
                ...
                if (this.form.id == null) {
                    result = axios.post(this.baseURL + '/level', this.form);
                } else {
                    result = axios.put(this.baseURL + '/level', this.form);
                }
                // 处理结果
                result.then(res => {
                    if (res.data.flag) {
